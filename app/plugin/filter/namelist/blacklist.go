...
type BlackNameList struct {
	host        string             // 主机地址
	bloomFilter *bloom.BloomFilter // 布隆过滤器
}

// Check 检查ip是否允许通行
func (l *BlackNameList) Check(ip string) (error, bool) {
	// 布隆过滤器校验，没命中则一定不存在
	...
	if err := global.MDB.Raw(`SELECT 1 
					...
					WHERE a.host = ? AND a.id = nl.app_id AND nl.genre = ? AND nl.ip = ?`, l.host, constants.NameList.Black.Value, ip).Scan(&res).Error; err != nil {
...
func (l *BlackNameList) Refresh() error {
	err, bloomFilter := buildBloomFilter(l.host, constants.NameList.Black.Value)
	if err != nil {
		return err
	}
...
func NewBlackNameList(host string) (error, *BlackNameList) {
	err, bloomFilter := buildBloomFilter(host, constants.NameList.Black.Value)
	if err != nil {
		return err, nil
	}
	return nil, &BlackNameList{
		host:        host,
