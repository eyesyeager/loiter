...
func (*appService) AddApp(r *http.Request, userClaims utils.JwtCustomClaims, data receiver.AddApp) error {
	...
	if err := global.MDB.Create(&newApp).Error; err != nil {
		errMsg := fmt.Sprintf(result.CommonInfo.DbOperateError, err.Error())
		global.AppLogger.Error(errMsg)
		return errors.New(errMsg)
	}
	// 插入默认负载均衡策略
	if err := BalancerService.AddAppBalancer(newApp.ID, config.Program.PluginConfig.BalancerDefaultStrategy); err != nil {
		errMsg := fmt.Sprintf(result.CommonInfo.DbOperateError, err.Error())
		global.AppLogger.Error(errMsg)
		return errors.New(errMsg)
	}
