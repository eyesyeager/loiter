...
func buildBloomFilter(host string, genre string) (error, *bloom.BloomFilter) {
	...
	if err := global.MDB.Raw(`SELECT nl.ip 
		...
		return errors.New(fmt.Sprintf(result.CommonInfo.DbOperateError, "buildBloomFilter()-Raw", err.Error())), nil
	}
	// 构建布隆过滤器
	bloomFilter := bloom.NewWithEstimates(config.Program.NameListBloomCapacity, config.Program.NameListBloomMisjudgmentRate)
